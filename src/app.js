...
const { PDF_DIR, listPurchases } = require('./db');
...
function createApp() {
  ...
  const PAYPAL_API = ['https://api-m.paypal.com', 'https://api-m.sandbox.paypal.com']; // endpoint REST API PayPal
  const PAYPAL_ASSETS = ['https://www.paypalobjects.com']; // asset statici SDK

  app.use(helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'"],
        scriptSrc: ["'self'", "'unsafe-inline'", ...PAYPAL_ORIGINS], // unsafe-inline necessario per SDK PayPal v5 (usa script inline interni)
        styleSrc: ["'self'", "'unsafe-inline'"], // style inline per PDF.js canvas
        imgSrc: ["'self'", 'data:', 'blob:', ...PAYPAL_ORIGINS, ...PAYPAL_ASSETS],
        fontSrc: ["'self'", 'data:'],
        workerSrc: ["'self'", 'blob:'],
        connectSrc: ["'self'", ...PAYPAL_ORIGINS, ...PAYPAL_API, ...PAYPAL_ASSETS], // SDK necessita endpoint REST API + assets
        ...
        popupSrc: ["'self'", ...PAYPAL_ORIGINS], // popup di checkout PayPal
        baseUri: ["'self'"],
        formAction: ["'self'"],
        // Trusted Types
  ...
  // I miei metodi (acquisti)
  app.get('/i-miei-metodi', (req, res) => {
    serveHtmlPage(req, res, 'purchases.html');
  });
